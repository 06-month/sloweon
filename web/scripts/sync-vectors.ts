...
async function upsertChunk(chunk: RagChunkInput) {
...
async function syncProducts() {
  ...
  for (const product of products) {
    ...
    const availableSizes = product.variants.filter(v => v.stock > 0 && v.status === "ON_SALE").map(v => `${v.colorName} ${v.size}`);
  ...
  console.log(`[sync] Synced ${products.length} products + product_detail chunks.`);
}

async function syncSizeSpecs() {
  console.log("[sync] Syncing size specs...");
  const products = await prisma.product.findMany({
    where: { status: "ON_SALE" },
  ...
  console.log(`[sync] Synced ${count} size guides.`);
}

async function syncReviews() {
  console.log("[sync] Syncing reviews...");
  const products = await prisma.product.findMany({
    where: { status: "ON_SALE" },
    include: {
      reviews: {
        where: { status: "PUBLISHED" },
        select: { rating: true, content: true, fitFeedback: true, purchasedSize: true, height: true, weight: true }
      }
    }
  });
  ...
  for (const product of products) {
    ...
    product.reviews.forEach(r => {
      if (r.fitFeedback in fitCounts) fitCounts[r.fitFeedback as keyof typeof fitCounts]++;
    });

    const reviewTexts = product.reviews.slice(0, 5).map(r => {
      const bodyInfo = r.height && r.weight ? `(${r.height}cm/${r.weight}kg, ${r.purchasedSize})` : `(${r.purchasedSize})`;
    ...
    await upsertChunk({
      ...
      content,
      metadata: { productId: product.id, avgRating: parseFloat(avgRating), reviewCount: product.reviews.length, fitCounts }
    });

    count++;
    await new Promise(r => setTimeout(r, 200));
  }
  console.log(`[sync] Synced ${count} review summaries.`);
}

async function syncPolicies() {
  console.log("[sync] Syncing policies...");
  ...
  for (const policy of policies) {
    await upsertChunk(policy);
    await new Promise(r => setTimeout(r, 200));
  }
  console.log(`[sync] Synced ${policies.length} policy/FAQ/brand chunks.`);
}
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      console.error("ERROR: rag_chunks table does not exist. Run the SQL migration first:");
      console.error("  psql $DATABASE_URL -f web/prisma/migrations/rag_pgvector_setup.sql");
      console.error("  or execute the SQL in Supabase SQL Editor.");
      process.exit(1);
    }
    ...
    console.log(`\n=== Sync Complete === Total chunks: ${totalResult[0]?.count || 0}`);
  } catch (error: any) {
